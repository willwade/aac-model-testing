...
def create_summary_sheet(workbook: Workbook, history: List[Dict[str, Any]]) -> None:
    ...
    for row, entry in enumerate(history, 2):
        ...
        ws.cell(
            row=row, column=6, value=round(best_score, 4) if best_score > 0 else "N/A"
        )
        ws.cell(row=row, column=7, value=round(avg_time, 2) if avg_time > 0 else "N/A")
        ws.cell(
            row=row, column=8, value=round(avg_memory, 1) if avg_memory > 0 else "N/A"
        )
...
def create_model_performance_sheet(
    workbook: Workbook, history: List[Dict[str, Any]], results_dir: Path
) -> None:
    """Create a detailed model performance comparison sheet with separate rows for each test type."""
    ws = workbook.create_sheet("Model Performance")

    # Collect all model performance data by test type
    performance_data = []
    ...
    for entry in history:
        ...
        # Load raw results to get per-test-type data
        raw_data = load_raw_results(results_dir, results_file)
        if not raw_data:
            continue

        # Extract per-test-type performance data
        for model_name, model_data in raw_data.get("results", {}).items():
            ...
            for test_type, test_data in test_results.items():
                ...
                # Get performance metrics for this test type from model-level metrics
                test_perf_metrics = performance_metrics.get(test_type, {})
                avg_response_time = test_perf_metrics.get("response_time", 0)
                peak_memory_mb = test_perf_metrics.get("peak_memory_mb", 0)

                # Get average memory from memory_usage_mb if available
                memory_usage_mb = test_perf_metrics.get("memory_usage_mb", {})
                avg_memory_mb = (
                    memory_usage_mb.get("average", 0)
                    if isinstance(memory_usage_mb, dict)
                    else 0
                )

                # Calculate test-specific score (average of individual test scores)
                ...
                avg_test_score = (
                    sum(test_scores) / len(test_scores) if test_scores else 0
                )
                ...
                performance_data.append(
                    {
                        ...
                        "Avg Test Score": round(avg_test_score, 4),
                        "Avg Response Time (s)": round(avg_response_time, 2),
                        "Peak Memory (MB)": round(peak_memory_mb, 1),
                        "Avg Memory (MB)": round(avg_memory_mb, 1),
                    }
                )
...
def create_detailed_responses_sheet(
    workbook: Workbook, history: List[Dict[str, Any]], results_dir: Path
) -> None:
            ...
            for test_case, test_data in test_results.items():
                ...
                for result in individual_results:
                    ...
                    row_data = [
                        ...
                        round(
                            result.get("metrics", {}).get("completeness_score", 0), 3
                        ),
                        round(result.get("metrics", {}).get("naturalness_score", 0), 3),
                        round(
                            result.get("metrics", {}).get("semantic_preservation", 0), 3
                        ),
                    ]
...
Examples:
  ...
  python generate_summary_xlsx.py --device work-laptop  # Filter by device
    ...
    parser.add_argument("--device", type=str, help="Filter results by device name")
    ...
    if args.device:
        history = [
            h
            for h in history
            if h.get("device_name", "").lower() == args.device.lower()
        ]
        if not history:
    ...
    try:
        ...
        create_model_performance_sheet(workbook, history, results_dir)
        ...
        print(
            f"🤖 Models tested: {len(total_models)} ({', '.join(sorted(total_models))})"
        )
