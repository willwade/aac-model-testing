...
# Import schemas - using try/except for robust importing
try:
    from ..schemas import PhraseboardResponse
except ImportError:
...
class PhraseboardTest(BaseTest):
    ...
    def _run_single_test_item(self, model, test_item: Dict[str, Any]) -> Dict[str, Any]:
        """Override to use structured output for phraseboard generation."""
        return self._run_single_test_item_structured(
            model, test_item, PhraseboardResponse
        )

    def _extract_response_text(self, structured_response: PhraseboardResponse) -> str:
        """Extract the phrases as CSV text for compatibility."""
        phrases = [phrase.text for phrase in structured_response.phrases]
        return "\n".join(phrases)
        ...
        return [
            {
                "id": "pb_001",
                "input": "dogs",
                "prompt": "Generate 12 words/phrases for topic: {input}",
                "expected_categories": [
                    "actions",
                    "descriptions",
                    "needs",
    ...
    def evaluate_response(
        ...
        evaluation["passed"] = (
            overall_score >= 0.7 and len(phrases) >= 10
        )  # Allow some flexibility

        # Generate feedback
        evaluation["feedback"] = self._generate_feedback(
            test_item, phrases, evaluation["metrics"]
        )
    ...
    def _evaluate_relevance(
        self, test_item: Dict[str, Any], phrases: List[str]
    ) -> float:
        ...
        for phrase in phrases:
            ...
            phrase_relevance = (
                0.3 * (1.0 if direct_match else 0.0) + 0.7 * semantic_relevance
            )
    ...
    def _calculate_topic_relevance(
        self, test_item: Dict[str, Any], phrase: str
    ) -> float:
        ...
        keyword_matches = sum(
            1 for keyword in topic_keywords if keyword in phrase_lower
        )
        keyword_score = (
            min(1.0, keyword_matches / len(topic_keywords)) if topic_keywords else 0.0
        )
    ...
    def _evaluate_diversity(
        ...
        if expected_categories:
            category_coverage = self._estimate_category_coverage(
                phrases, expected_categories
            )
        else:
    ...
    def _estimate_category_coverage(
        self, phrases: List[str], expected_categories: List[str]
    ) -> float:
        ...
        coverage = (
            len(covered_categories.intersection(expected_set)) / len(expected_set)
            if expected_set
            else 0.0
        )
    ...
    def _generate_feedback(
        self, test_item: Dict[str, Any], phrases: List[str], metrics: Dict[str, float]
    ) -> str:
        ...
        if phrase_count == 12:
            ...
            feedback_parts.append(
                f"Good! Generated {phrase_count} phrases (close to the requested 12)."
            )
        else:
            feedback_parts.append(
                f"Generated {phrase_count} phrases. Aim for exactly 12."
            )
        ...
        if metrics["aac_suitability_score"] >= 0.8:
            ...
            feedback_parts.append(
                "Phrases could be more suitable for AAC users (shorter, more functional)."
            )
    ...
    def get_test_description(self) -> str:
        ...
        return (
            "Phrase Board Test - Evaluates the model's ability to generate "
