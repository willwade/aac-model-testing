...
try:
    from ..schemas import UtteranceSuggestionsResponse
except ImportError:
...
class UtteranceSuggestionTest(BaseTest):
    ...
    def _run_single_test_item(self, model, test_item: Dict[str, Any]) -> Dict[str, Any]:
        """Override to use structured output for utterance suggestions."""
        return self._run_single_test_item_structured(
            model, test_item, UtteranceSuggestionsResponse
        )

    def _extract_response_text(
        self, structured_response: UtteranceSuggestionsResponse
    ) -> str:
        """Extract the suggestions as comma-separated text for compatibility."""
        suggestions = [
            suggestion.text for suggestion in structured_response.suggestions
        ]
    ...
    def _parse_phrases(self, response: str) -> List[str]:
        ...
        comma_split = [
            phrase.strip() for phrase in cleaned.split(",") if phrase.strip()
        ]
    ...
    def _evaluate_relevance(
        ...
        for phrase in phrases:
            ...
            keyword_matches = sum(
                1 for keyword in input_keywords if keyword in phrase_words
            )
    ...
    def _is_complete_sentence(self, phrase: str) -> bool:
        ...
        has_pronoun = any(
            word in words
            for word in ["i", "you", "he", "she", "it", "we", "they", "my", "your"]
        )
        has_verb = any(
            word in words
            for word in [
    ...
    def _generate_feedback(
        ...
        if phrase_count == 0:
            return (
                "No valid phrases were generated. Please provide 3-5 complete phrases."
            )

        if phrase_count < 3:
            feedback_parts.append(
                f"Only {phrase_count} phrase(s) generated. Aim for 3-5 phrases."
            )
        elif phrase_count > 5:
            feedback_parts.append(
                f"{phrase_count} phrases generated. 3-5 is ideal for AAC users."
            )
        else:
        ...
        if metrics["relevance_score"] >= 0.8:
            ...
            feedback_parts.append(
                "Phrases could be more relevant to the input keywords."
            )
    ...
    def get_test_description(self) -> str:
        ...
        return (
            "Utterance Suggestion Test - Evaluates the model's ability to generate "
