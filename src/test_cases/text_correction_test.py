...
try:
    from ..schemas import TextCorrectionResponse
except ImportError:
...
class TextCorrectionTest(BaseTest):
    ...
    def _run_single_test_item(self, model, test_item: Dict[str, Any]) -> Dict[str, Any]:
        """Override to use structured output for text correction."""
        return self._run_single_test_item_structured(
            model, test_item, TextCorrectionResponse
        )

    def _extract_response_text(
        self, structured_response: TextCorrectionResponse
    ) -> str:
        """Extract the corrected text from the structured response."""
        return structured_response.corrected_text
        ...
        return [
            {
                ...
                "prompt": "Fix this grammatically incorrect sentence: {input}",
    ...
    def _generate_feedback(
        self, test_item: Dict[str, Any], response: str, metrics: Dict[str, float]
    ) -> str:
        ...
        if metrics["completeness_score"] < 0.7:
            feedback_parts.append(
                "Some key information from the original text may be missing."
            )
    ...
    def get_test_description(self) -> str:
        ...
        return (
            "Text Correction Test - Evaluates the model's ability to correct "
